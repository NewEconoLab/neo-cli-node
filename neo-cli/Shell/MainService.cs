...
namespace Neo.Shell
{
    internal class MainService : ConsoleServiceBase
    {
        ...
        protected internal override void OnStart(string[] args)
        {
            ...
            MongoDBHelper.InnerDB.dbConnStr = Settings.Default.dbInfo.dbConnStr;
            MongoDBHelper.InnerDB.dbDatabase = Settings.Default.dbInfo.dbDataBase;
            MongoDBHelper.InnerDB.leveldbCol = Settings.Default.dbInfo.leveldbCol;
        ...
        private void LevelDBBlockchain_ApplicationExecuted(object sender, ApplicationExecutedEventArgs e)
        {
            ...
            // 入库notify
            string dbConnStr = Settings.Default.dbInfo.dbConnStr;
            string dbDatabase = Settings.Default.dbInfo.dbDataBase;
            string notifyCol = Settings.Default.dbInfo.notifyCol;
            json["blockindex"] = e.ExecutionResults[0].blockindex;
            MongoDBHelper.PutData(dbConnStr, dbDatabase, notifyCol, json.ToString());
        }

        private bool checkBlockAndTx(ApplicationExecutedEventArgs e)
        {
            string data = null;
            if(e.block != null)
            {
                ...
                string dbConnStr = Settings.Default.dbInfo.dbConnStr;
                string dbDatabase = Settings.Default.dbInfo.dbDataBase;
                string blockCol = Settings.Default.dbInfo.blockCol;
                string txCol = Settings.Default.dbInfo.txCol;
                // 入库Block
                if(MongoDBHelper.GetDataCount(dbConnStr, dbDatabase, blockCol, "{'index':"+e.block.Index+"}") <= 0)
                {
                    MongoDBHelper.PutData(dbConnStr, dbDatabase, blockCol, data);
                }
                // 入库Tx
                if (MongoDBHelper.GetDataCount(dbConnStr, dbDatabase, txCol, "{'blockindex':" + e.block.Index + "}") <=0)
                {
